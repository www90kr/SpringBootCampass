...
import com.campass.demo.dto.Mail;

@Component
public class SellerMailUtil {
	@Autowired
	...
	// 임시 비밀번호 보내는 메소드
	public void sendResetPasswordMail(String from, String to, String spassword) {
		Mail mail = Mail.builder().from(from).to(to).subject("임시비밀번호").build();
		String message = new StringBuffer("<p>임시비밀번호를 발급했습니다</p>").append("<p>임시비밀번호 :").append(spassword).append("</p>").toString();
