...
@Component
public class BuyerLoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
	@Autowired
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if(request.getParameter("password").length()>=20) {
			...
			rs.sendRedirect(request, response, "/buyer/changePassword");
			...
			if(savedRequest!=null)
				...
				rs.sendRedirect(request, response, "/buyer/loginsuc");	
		}
	}
